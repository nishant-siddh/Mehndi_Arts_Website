      ...
      >
        ...
        <div className={`w-full sm:w-fit m-0 absolute top-[5.3rem] left-0 sm:relative sm:top-0 bg-[#4E6659] ${isNavOpen ? 'py-5' : 'py-0'} transition-all duration-300`}>
          <ul className={`list ${isNavOpen ? "flex flex-col gap-5 mx-auto" : 'hidden'} sm:flex-row justify-between items-center gap-10 sm:flex text-[#FFFFF0]/90 hover: text-[#FFFFF0]`}>
            ...
            <li>
              <div className="flex items-center cursor-pointer" onClick={() => setIsServicesOpen(prev => !prev)}>
                <NavLink to="#">Services</NavLink>
                <span className="text-lg">{isServicesOpen ? <IoMdArrowDropup /> : <IoMdArrowDropdown />}</span>
              </div>
              <div className={`absolute bg-[#FFFFF0] z-50 top-10 sm:top-14 left-0 w-44 text-[#4E6659] flex-col ${isServicesOpen ? 'flex' : 'hidden'}`}>
                {services.map((service) => (
                  <>
                    <span className="py-2 px-3 hover:bg-[#e7a316] hover:text-[#FFFFF0]">
                      <NavLink to={`/services/${service.to}`} onClick={() => { setIsServicesOpen(false), setIsNavOpen(false) }}>
                        {service.name}
                      </NavLink>
                    </span>
                  </>
        ...
        <div className="flex sm:hidden" onClick={() => setIsNavOpen(prev => !prev)}>
          {!isNavOpen ? <GiHamburgerMenu className="text-[#FFFFF0] text-lg" /> : <RxCross1 className="text-[#FFFFF0] text-lg" />}
        </div>
      </div>
    </nav>
  );
